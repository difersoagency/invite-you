...
export default function Page({ params }: { params: { id: string } }) {
  ...
  const onSubmit = () => {
    if (template == 0) {
      ...
      router.push(`/create/birthday/detail/${params.id}`);
    }
  };
    ...
    <>
      <section>
        ...
        <div className="px-10 py-7 text-center">
          ...
          <div className="mt-10 grid grid-cols-1 md:grid-cols-3 h-[60vh] gap-11 overflow-y-scroll mb-7">
            {templateList.map((option) => (
              ...
              <div key={option.id}>
                ...
                <label
                  htmlFor={option.id}
                  className="inline-flex items-center justify-between w-full p-5 text-dark bg-white peer-checked:border rounded-lg cursor-pointer  peer-checked:border-gold peer-checked:text-gold peer-checked:bg-cream hover:text-gold hover:bg-cream  "
                >
                  <div className="block">
                    <img
          ...
          <button
            className="px-6 mx-auto py-2 text-xs bg-gold text-white rounded-md"
            onClick={onSubmit}
          >
            Next
          </button>
